...
INT ExplorerInject(HANDLE ownerProcess)
{
	...
	hLoadedDll = LoadLibraryInExplorer(hExplorerProcess, szDllFileName, hRemoteEvent, &pExitProc, ownerProcess);
	if (hLoadedDll)
	{
		...
		if (hRemoteWaitThread)
		{
			...
			pError = IDS_INJERROR_X3;
	}
	else
		pError = IDS_INJERROR_LOADDLL;
...
static HMODULE LoadLibraryInExplorer(HANDLE hProcess, WCHAR *pDllName, HANDLE hEvent, void **ppExitProc, HANDLE ownerProcess)
{
	...
	if(
		...
		!MyWriteProcessMemory(hProcess, (char*)pInjectAddr + sizeof(ASM_CODE_LOAD) - 1 + nDllNameSize, &ownerProcess, sizeof(HWND))
	)
	{
